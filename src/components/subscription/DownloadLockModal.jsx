...
const COPY = {
  ...
  // A teacher added by a School Admin shares the school's plan and can't buy
  // their own — no price, no CTA, just a pointer to whoever can act on it.
  schoolTeacher: {
    title: 'Download Locked',
    message: "Your school's free downloads are complete.",
    pitch: 'Ask your school admin to upgrade to School Pro for unlimited downloads.',
    benefits: [],
  },
}
...
export function DownloadLockModal() {
  ...
  const copy = isSchoolLinkedTeacher ? COPY.schoolTeacher : (COPY[accountType] || COPY.teacher)
    ...
    <Dialog
      ...
      footer={
        copy.cta ? (
          ...
          <Button onClick={closeDownloadLock}>Got it</Button>
        )
      }
    >
      <p className="mb-1">{copy.message}</p>
      {copy.pitch && <p className="mb-4 text-ink-500 dark:text-ink-400">{copy.pitch}</p>}
      {copy.price && <p className="mb-3 font-display text-2xl font-semibold text-ink-900 dark:text-ink-50">{copy.price}</p>}
